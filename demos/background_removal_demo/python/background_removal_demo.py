...
def main():
    ...
    def make_transparent_foreground(frame, mask):
          # split the image into channels
          b, g, r = cv2.split(np.array(frame).astype('uint8'))
    ...
    def remove_background(frame,mask):
      ...
      foreground = make_transparent_foreground(frame ,bin_mask)
    ...
    while True:
        ...
        if results:
            ...
            #frame = objects
            frame,bin_mask = remove_background(frame,objects)
            #bin_mask,frame = remove_background(frame,objects)
            #frame=custom_background("D:\open_model_zoo\demos\segmentation_demo\python\custom_back.jpg", frame)
