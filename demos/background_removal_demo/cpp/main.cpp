...
static void showUsage() {
    std::cout << std::endl;
    std::cout << "Background Removal Demo [OPTION]" << std::endl;
...
cv::Mat Foreground(const ImageResult& result, OutputTransform& outputTransform) {
    if (!result.metaData) {
        throw std::invalid_argument("Renderer: metadata is null");
    }
    // Input image is stored inside metadata, as we put it there during submission stage
    ...
    for (int row = 0; row < Mask.rows; ++row)
    {
        for (int col = 0; col < Mask.cols; ++col) {
            ...
            {
                bin_mask.at<uchar>(row, col) = background.at<uchar>(row, col);
            }
        }
    }
    ...
    cv::split(inputImg, different_Channels);
    cv::Mat b = different_Channels[0];
    cv::Mat g = different_Channels[1];
    cv::Mat r = different_Channels[2];
    // add an alpha channel with and fill all with transparent pixels(max 255)

    cv::Mat a(bin_mask.size(), CV_8UC1, cv::Scalar(255));
    // merge the alpha channel back
    cv::Mat alpha_im;
    cv::Mat channels[4] = { b,g,r,a };
    cv::merge(channels, 4, alpha_im);
    //slog::debug << " alpha Image shape --" << alpha_im.size() << slog::endl;


    //create a transparent background
    //cv::Mat bg = cv::Mat::zeros(alpha_im.size(), CV_8UC4); // for solid black
    cv::Mat bg;
    blur(alpha_im, bg, cv::Size(21, 21)); // for blur 
    //setup the new mask
    cv::Mat new_mask;
    cv::Mat allmask[4] = { bin_mask, bin_mask, bin_mask, bin_mask };
    cv::merge(allmask, 4, new_mask);
    //slog::debug << " new Mask shape --" << new_mask.size() << slog::endl;

    // copy only the foreground color pixels from the original image where mask is set

    cv::Mat foreground = cv::Mat::zeros(alpha_im.size(), CV_8UC4);


    for (int row = 0; row < new_mask.rows; ++row)
    {
        for (int col = 0; col < new_mask.cols; ++col) {

            for (int ch = 0; ch < 4; ++ch)
            {
                if (new_mask.at<cv::Vec4b>(row, col)[ch])
                {
                    foreground.at<cv::Vec4b>(row, col)[ch] = alpha_im.at<cv::Vec4b>(row, col)[ch];
                }
                else
                {
                    foreground.at<cv::Vec4b>(row, col)[ch] = bg.at<cv::Vec4b>(row, col)[ch];
                }
            }
    ...
    try
    {
        ...
        while (keepRunning) {
            ...
            while (keepRunning && (result = pipeline.getResult())) {
                ...
                cv::Mat outFrame = Foreground(result->asRef<ImageResult>(), outputTransform);
